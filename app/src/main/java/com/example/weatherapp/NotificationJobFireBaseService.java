...
public class NotificationJobFireBaseService extends JobService {
    ...
    public boolean onStartJob(JobParameters job) {
        ...
        pendingIntent = PendingIntent.getBroadcast(this, 111, intent, PendingIntent.FLAG_UPDATE_CURRENT);
        /**
         * Intent 플래그
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            manager.setExactAndAllowWhileIdle(AlarmManager.RTC_WAKEUP, Alarm.calendar.getTimeInMillis(), pendingIntent); //10초뒤 알람
        } else if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            manager.setExact(AlarmManager.RTC_WAKEUP,Alarm.calendar.getTimeInMillis(), pendingIntent);
        } else {
            manager.set(AlarmManager.RTC_WAKEUP,Alarm.calendar.getTimeInMillis(), pendingIntent);
        }
